...
void gpt1_isr() {
  if (GPT1_SR & GPT_SR_IF1) { // Input capture 1 event
    uint32_t cap = GPT1_ICR1;
    uint32_t prev = hw_lastEdgeCapture;
    if (prev != 0) {
      uint32_t diff = (cap >= prev) ? (cap - prev) : ((0xFFFFFFFFu - prev) + cap + 1u);
      hw_lastPeriodTicks = diff;
    }
    hw_lastEdgeCapture = cap;
    hw_lastEdgeTimeCycles = readCycleCounter();
    GPT1_SR = GPT_SR_IF1; // Clear flag
  }
}

void configureHardwareMeasurement() {
  // Enable GPT1 clock
  CCM_CCGR1 |= CCM_CCGR1_GPT1(CCM_CCGR_ON);
  GPT1_CR = 0;
  GPT1_PR = 0; // Prescaler = 0 (24 MHz clock)
  GPT1_OCR1 = 0xFFFFFFFF;
  GPT1_ICR1 = 0;
  GPT1_CR = GPT_CR_EN | GPT_CR_CLKSRC(1); // Peripheral clock (24 MHz)
  // Configure pin 21 (GPIO_AD_B1_11) for GPT1 capture
  IOMUXC_SW_MUX_CTL_PAD_GPIO_AD_B1_11 = 3; // ALT3 = GPT1_CAPTURE1
  // Enable input capture 1 interrupt
  attachInterruptVector((IRQ_NUMBER_t)IRQ_GPT1, gpt1_isr);
...
void setup() {
  ...
  // Configure hardware measurement
  configureHardwareMeasurement();
